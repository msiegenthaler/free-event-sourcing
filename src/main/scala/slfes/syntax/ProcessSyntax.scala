...
object ProcessSyntax {
  ...
  def await[A <: AggregateInterface, For <: Coproduct, R](handler: Handler[A, For, R]): ProcessBodyM[R] =
  ...
  case class From[A <: AggregateInterface](id: A#Id) {
    def event[E]: OnFirst[A, E] = new OnFirst[A, E](id)
  }
  case class OnFirst[A <: AggregateInterface, E](id: A#Id) {
  ...
  case class Handler[A <: AggregateInterface, For <: Coproduct, R](id: A#Id, handle: AggregateEvt[A] ⇒ Option[R]) {
  ...
  def spawnFor[A <: AggregateInterface](aggregate: A) = SpawnFor(aggregate)
  case class SpawnFor[A <: AggregateInterface](aggregate: A) {
    def on[E] = SpawnOn[A, E](aggregate)
  }
  case class SpawnOn[A <: AggregateInterface, E](aggregate: A) {
  ...
  case class ProcessName(name: String) {
    def startedAt[A <: AggregateInterface](aggregate: A) = ProcessStartedAt(name, aggregate)
  }
  case class ProcessStartedAt[A <: AggregateInterface](name: String, aggregate: A) {
    def on[E] = ProcessSpawnOn[A, E](name, aggregate)
  }
  case class ProcessSpawnOn[A <: AggregateInterface, E](name: String, aggregate: A) {
